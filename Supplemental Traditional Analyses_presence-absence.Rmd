---
title: "Supplemental Traditional Analyses, presence-absence transformed"
author: 
...
for(ds_name in names(coda.clrs.res[grep("_3", names(coda.clrs.res), value = T)])){
        ...
        ##PAIRWISE PERMANOVA
        adon.res.tmp <- ortho_adonis(d.clr.abund, topo.contrast.matrix, c("topo"), char_data,  method = distmetric) %>%
           mutate(dataset = ds_name, Parameter = "") %>% rbind(adon.res.tmp)
        
        ##PAIRWISE BETADISPER
        
          betadisper.res.tmp <- ortho_betadisper(d.clr.abund, topo.contrast.matrix, c("topo"), char_data,  method = distmetric) %>%
           mutate(dataset = ds_name, Parameter = "") %>% rbind(betadisper.res.tmp)
