...
for(ds_name in names(coda.clrs.res[grep("_1", names(coda.clrs.res), value = T)])){
   ...
   geom_polygon(data=hulls, aes(group=hullgrp, alpha = harvest, color=harvest)) +
...
## plot as boxplots with significance
     ggplot(plot.betadisper %>% mutate(dataset = gsub("_[[:digit:]]", "", dataset)), aes(harvest, distance, color = harvest))+
     geom_boxplot()+
       geom_text(data= temp.sig, inherit.aes = F, aes(5.75, distance*0.9, label = sig.level, color = text.col))+
           geom_segment(data = temp.sig, inherit.aes = F, aes(y = distance*0.9, yend = distance*0.9, x = firstseg.start, xend = firstseg.end, color = text.col, size = text.col)) +
           geom_segment(data = temp.sig, inherit.aes = F, aes(y = distance*0.9, yend = distance*0.9, x = secondseg.start, xend = secondseg.end, color = text.col, size = text.col)) +
       theme_minimal()+
       scale_color_manual(values= c(harv.cols, "significant at p-value < 0.05" = "red", "non-significant" = "black")) +
       scale_size_manual(name = "significance", values= c(1,1), guide = guide_legend(override.aes = list(colour = c("red", "black")))) +
...
for(ds_name in names(coda.clrs.res[grep("_3", names(coda.clrs.res), value = T)])){
   ...
   geom_polygon(data=hulls, aes(group=hullgrp, color=topo), fill= NA) +
...
for(ds_name in names(coda.clrs.res[grep("_3", names(coda.clrs.res), value = T)])){
   ...
   geom_point(data=plottingdat, aes(color=topo, shape = topo), size = 2) + 
   geom_polygon(data=hulls, aes(group=hullgrp, color=topo), fill = "#FFFFFF") +
...
ggplot(div.res%>%
  mutate(dataset = str_remove(dataset, "_2")), aes(month, value, col = harvest)) +
...
## plot as boxplots with significance
  ...
  mutate(dataset = str_remove(dataset, "_2")), aes(month, distance, color = harvest))+
     geom_boxplot()+
